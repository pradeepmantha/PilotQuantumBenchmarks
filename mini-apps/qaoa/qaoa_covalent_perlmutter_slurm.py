...
perlmutter_slurm = ct.executor.HPCExecutor(
    address="perlmutter-p1.nersc.gov",
    username="prmantha",
    ssh_key_file="~/.ssh/nersc",
    cert_file="~/.ssh/nersc-cert.pub",
    remote_conda_env="py39",
    instance="slurm",
    cleanup=False,
    remote_workdir="/pscratch/sd/p/prmantha/covalent-workdir",
    log_stdout="stdout",
    log_stderr="stderr",
    job_attributes_kwargs={
        "project_name": "m4408",
        "custom_attributes": {"slurm.constraint": "cpu", "slurm.qos": "regular"},
    },
)
@ct.electron
...
@ct.lattice(executor=perlmutter_slurm)
def workflow(
    p=1,
    qubits=2,
    prob=0.3,
...
def run_workflow(
    range_qubits: List[int] = [32],
    range_iterations: List[int] = [1000],
):
    all_results = []
